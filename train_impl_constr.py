...
from constraint import pos_impl_constraint
...
def softmax_constraint(logits, token):
    """
...
#                     con += 1 - pos_impl_constraint(probs[:,pos,-7], probs[:, pos+1, -19]) #t, h
#                 c.append(con)
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        # constraints = estimate_constraints([lambda logits: softmax_constraint(logits, token) for token in BANNED_TOKENS])
    ...
    with ctx:
        ...
        # for token in BANNED_TOKENS:
            # constrain just conditional prob ?
            # p_t1_pos = probs[:,pos,-7]
            # p_t1_pos = probs[:,pos,-7]


            # con += torch.mean(pos_impl_constraint(probs[:,pos,-7], probs[:, pos+1, -19]), dim=0)
            con += torch.mean(pos_impl_constraint(probs[:,pos,0], probs[:, pos+1, -13])  - 1, dim=0)
            # con += torch.mean(pos_impl_constraint(probs[:,pos,-7], probs[:, pos+1, -19])  - 1, dim=0)
